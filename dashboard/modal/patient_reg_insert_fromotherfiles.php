...
$iname = $_FILES['image']['name'];
  ...
  $inamek1 = $_POST['name1'];
	...
	//move_uploaded_file($tmp,"../staff/" . $code.$_FILES["sfile"]["name"]);
	 $iname1 =$code. $_FILES['image']['name'];
	//$iname = $code.$_FILES["sfile"]["name"];
	$iname1 = mysqli_real_escape_string($iname1);
	}else{
	 $iname1 = mysqli_real_escape_string($img);
	}
...
date_default_timezone_set('Asia/Kolkata');
 ...
 `gender`, `address`, `phoneno`, `registerfee`, `remarks`, `pat_type`, `pay_type`, `aadar`, `ref_doc`, `ref_doc_mob`,
  ...
  bill_num,serv_name,hosp_fee,reg_no,opt_type,rec_no,image,visit_count_pat,have_ref) VALUES 
 ...
 '$token1','$ser','$cnt','$serv_name','$ser_amt','$reg_no','$appoint_type','$rec_no','$iname1','1','$have_ref')"; 
